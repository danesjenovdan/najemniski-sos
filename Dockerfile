...
COPY ./stanovanja/requirements.txt /
...
COPY --chown=wagtail:wagtail ./stanovanja .

# Copy compiled css from other image
COPY --chown=wagtail:wagtail --from=sass-compile /app/static/css ./stanovanja/static/css

# Use user "wagtail" to run the build commands below and the server itself.
USER wagtail

CMD gunicorn stanovanja.wsgi:application -b 0.0.0.0:8000 --log-level DEBUG
